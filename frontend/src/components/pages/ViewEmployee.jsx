...
const ViewEmployee = () => {
  ...
  const fetchEmployee = async () => {
    try {
      ...
      // Filter out the history for unchanged fields like `joiningDate` and `resignationDate`
      const filteredHistory = res.data.employee.history?.filter((record) => {
        // Only include records where `field` is different from `joiningDate` or `resignationDate`
        return record.field !== "joiningDate" && record.field !== "resignationDate";
      }) || [];
      setHistory(filteredHistory); // Set filtered history
    } catch (error) {
